...
output. The usage bar extension in vscode/ reads the file that this hook
writes: ~/.claude/usage/<session_id>.json, or the same path under
CLAUDE_CONFIG_DIR when that is set. One folder holds the sessions of every
project on the machine. The kit plugin ships this hook in hooks/hooks.json, so
it runs in every project, and each file records the project directory of its
session.
...
def config_dir() -> str:
    """Claude Code's own settings folder: CLAUDE_CONFIG_DIR when set, else ~/.claude."""
...
def main() -> None:
    ...
    usage_dir = os.path.join(config_dir(), "usage")
