// Shows the token, prompt cache, cost and graft figures of the live Claude Code sessions on this machine.
// The status bar item shows the newest session that runs inside a folder of this window, or the newest
// session on the machine when none does. The hover lists every live session, and a click opens a
// details pane with all of them.
//
// The Claude Code panel does not run a statusLine command. The kit plugin's hook, hooks/session-usage.py,
// writes the figures of each session to ~/.claude/usage/<session_id>.json with the session's project
// directory. graft writes its own figures to graft/.cache/session/<session_id>.json inside that project.
// This extension reads both.
...
const DEFAULT_WARN_TOKENS = 500000;
const DEFAULT_ALERT_TOKENS = 750000;
// The three steps that move a long session to a fresh one. The pane, the hover and the notification all say them.
const HANDOFF_STEPS = 'run /kit:handoff, then /clear, then say "continue from handoff <topic>"';
...
// The two context sizes where the colors change: the warning color at the first, the error color at the second.
function lines() {
  const config = vscode.workspace.getConfiguration('claudeUsageBar');
  return {
    warn: config.get('warnTokens', DEFAULT_WARN_TOKENS),
    alert: config.get('alertTokens', DEFAULT_ALERT_TOKENS),
  };
}

function barColor(context) {
  const { warn, alert } = lines();
  if (context >= alert) return new vscode.ThemeColor('statusBarItem.errorBackground');
  if (context >= warn) return new vscode.ThemeColor('statusBarItem.warningBackground');
  return undefined;
}

// The folder the hook writes to: usage/ under CLAUDE_CONFIG_DIR when set, else under ~/.claude.
function usageDir() {
  return path.join(process.env.CLAUDE_CONFIG_DIR || path.join(os.homedir(), '.claude'), 'usage');
}

// True when the second directory is the first one or lies inside it.
function sameTree(outer, inner) {
  const rel = path.relative(outer, inner);
  return rel === '' || (!rel.startsWith('..') && !path.isAbsolute(rel));
}

// A session belongs to this window when its project directory and a workspace folder share a tree.
function inWindow(projectDir) {
  if (!projectDir) return false;
  return (vscode.workspace.workspaceFolders ?? []).some((folder) => {
    const root = folder.uri.fsPath;
    return sameTree(root, projectDir) || sameTree(projectDir, root);
  });
}
...
// Every session on the machine that is not ended and was written in the last 24 hours.
// The sessions of this window come first. Within each group the newest comes first.
function loadSessions(now) {
  ...
  for (const name of names) {
    ...
    const projectDir = usage.project_dir ?? '';
    const graft = (projectDir && readJson(path.join(projectDir, 'graft', '.cache', 'session', `${usage.session_id}.json`))) || {};
    sessions.push(summarize(usage, graft, {
      file,
      mtime,
      now,
      projectDir,
      folder: path.basename(projectDir) || 'unknown folder',
      inWindow: inWindow(projectDir),
    }));
  }
...
function hover(sessions) {
  ...
  const md = new vscode.MarkdownString([
    ...
    '',
    ...(overWarn ? [`Above the warning line: ${HANDOFF_STEPS}.`, ''] : []),
    'A folder in bold is open in this window. Click to open the details pane.',
  ].join('\n'));
...
let panel = null;
let sessions = [];
// The session and line pairs that already got their notification, so each comes once.
const notified = new Set();

// One notification per session and per line, for the sessions of this window only.
// A session of another window gets its notification in that window.
function notifyHandoff(list) {
  const { warn, alert } = lines();
  for (const s of list) {
    if (!s.inWindow) continue;
    const line = s.context >= alert ? 'alert' : s.context >= warn ? 'warn' : null;
    if (!line || notified.has(`${s.id}:${line}`)) continue;
    notified.add(`${s.id}:${line}`);
    const method = line === 'alert' ? 'showErrorMessage' : 'showWarningMessage';
    const text = `Claude session ${s.short} in ${s.folder} holds ${tokens(s.context)} tokens of context. In the Claude Code panel, ${HANDOFF_STEPS}.`;
    vscode.window[method](text, 'Show sessions').then((choice) => {
      if (choice) vscode.commands.executeCommand(DETAILS_COMMAND);
    });
  }
}

function postSessions() {
  if (panel) panel.webview.postMessage({ type: 'sessions', sessions, lines: lines() });
}
...
function render(item) {
  ...
  item.backgroundColor = barColor(current.context);
  item.show();
  postSessions();
}
...
<style nonce="${n}">
  ...
  .meter-alert .meter-fill { background: var(--vscode-charts-red); }
  ...
  .meter-alert .meter-foot { color: var(--vscode-errorForeground); }
...
<script nonce="${n}">
  ...
  const HANDOFF_STEPS = ${JSON.stringify(HANDOFF_STEPS)};
  ...
  // The bar fills toward the two lines. The scale grows past the second only when the context does.
  function meter(s, lines) {
    const { warn, alert } = lines;
    const scale = Math.max(s.context, Math.round(alert / 0.9));
    const over = s.context >= warn;
    const overAlert = s.context >= alert;
    const wrap = el('div', 'meter' + (overAlert ? ' meter-alert' : over ? ' meter-over' : ''));
    ...
    track.append(fill);
    for (const [name, at] of [['Warning line', warn], ['Alert line', alert]]) {
      const tick = el('div', 'meter-tick');
      tick.style.left = ((at / scale) * 100).toFixed(1) + '%';
      tick.title = name + ': ' + count(at) + ' tokens';
      track.append(tick);
    }
    const note = overAlert
      ? 'Alert: above the ' + tokens(alert) + ' line. Every step re-reads all of it. Now ' + HANDOFF_STEPS + '.'
      : over
        ? 'Warning: above the ' + tokens(warn) + ' line. Every step re-reads all of it. Time to ' + HANDOFF_STEPS + '.'
        : 'Warning line at ' + tokens(warn) + ' tokens, alert line at ' + tokens(alert) + '.';
  ...
  function card(s, lines, isCurrent, wasOpen) {
    ...
    if (isCurrent) title.append(el('span', 'badge', 'in the status bar'));
    if (s.inWindow) title.append(el('span', 'badge', 'this window'));
    ...
    box.append(head, meter(s, lines), tiles, details);
    return box;
  }
  ...
  function render(state) {
    ...
    list.forEach((s, i) => root.append(card(s, state.lines, i === 0, opened.has(s.id))));
  }
...
function activate(context) {
  ...
  const dir = usageDir();
  try {
    // The watcher needs the folder to exist before the first hook of the day writes to it.
    fs.mkdirSync(dir, { recursive: true });
  } catch {
    // A home folder that refuses the write. The timer still refreshes the list.
  }
  // The usage folder lies outside the workspace, so the watcher takes a folder and a pattern, not a glob.
  const usageWatcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(vscode.Uri.file(dir), '*.json'));
  // graft's file lives inside each project. This watches the projects of this window; the other
  // projects refresh with their usage file, which changes on every tool call, and with the timer.
